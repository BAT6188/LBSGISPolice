...
namespace LBSgisPoliceEdit
{
    public partial class frmMap : Form
    {
        ...
        //private string GafileName = "";//用于公安数据编辑的fileName变量  feng
        ...
        public frmMap(string region1,string region2,DataTable dt)   //构造函数
        {
            try
            {
                InitializeComponent();
                ...
                //初始化默认表的编辑字段
                //this.InitialEditFields(comboTables.Text);//这个是用来初使化要输入的字段用的
        ...
        //添加民警中队辖区视图 (add by fisher in 09-12-01)
        ...
        void Tools_FeatureChanging(object sender, FeatureChangingEventArgs e)
        {
            try
            {
                ...
                {
                    //if (comboTables.Text == "视频位置") {  //视频位置不允许删除
        ...
        private void setPrivilege()    //基础数据设置权限
        {
            try
            {
                if (temEditDt.Rows[0]["基础数据可编辑"].ToString() != "1")
                {
                    ...
                    //this.dataGridViewVideo.ReadOnly = true;   //设置视频点的信息为只读
                }                
            }
            catch (Exception ex)
            {
        ...
        void Tools_Used(object sender, ToolUsedEventArgs e)
        {
            ...
            try
            {
                if (e.ToolName == "Location")
                {
                    ...
                    if (this.tabControl2.SelectedTab == tabPage1)  //基础业务数据
                    {
                    ...
                    else if (this.tabControl2.SelectedTab == tabPage2)  //视频定位
                    {
                    ...
                    //added by fisher (09-09-03)
                    else if (this.tabControl2.SelectedTab == tabPage3)   //公安业务数据
                    {
                            ...
                            case MapInfo.Tools.ToolStatus.Start:
                                ...
                                if (this.dataGridViewGaInfo.Rows[dataGridViewGaInfo.Rows.Count - 2].Cells[1].Value.ToString() != "" && this.dataGridViewGaInfo.Rows[dataGridViewGaInfo.Rows.Count - 1].Cells[1].Value.ToString() != "")
                                {
                                    if (Convert.ToDouble(this.dataGridViewGaInfo.Rows[dataGridViewGaInfo.Rows.Count - 2].Cells[1].Value) != 0 && Convert.ToDouble(this.dataGridViewGaInfo.Rows[dataGridViewGaInfo.Rows.Count - 1].Cells[1].Value) != 0)
                                    {
                                        ...
                                        //else
                                        //{
                                        //    SearchInfo si = MapInfo.Data.SearchInfoFactory.SearchWhere("编号='" + this.dataGridViewGaList.Rows[rowIndex].Cells["编号"].Value.ToString() + "'");
                                        //    si.QueryDefinition.Columns = null;
                                        //    f = MapInfo.Engine.Session.Current.Catalog.SearchForFeature(editTable.Alias, si);
                                        //    if (f != null)
                                        //    {
        ...
        private void labeLayer(Table table)//创建标注,在创建之前要删除以前的标注图层
        {
            try
            {
        ...
        private void InitialEditFields(string tabName)
        {
            ...
            try
            {
                ...
                for(int i=0;i<dt.Rows.Count;i++)
                {
                    ...
                    if (fieldName.ToUpper() == "MAPID" || fieldName.ToUpper() == "X" || fieldName.ToUpper() == "Y" || fieldName.IndexOf("备用字段") > -1 || fieldName.ToUpper() == "MI_STYLE" || fieldName.ToUpper() == "MI_PRINX" || fieldName.ToUpper() == "GEOLOC")
                    {
                        k++;
                        continue;
                    }
        ...
        private DataGridViewComboBoxCell dgvComboBoxJieZhen() 
        {
            try
            {
                ...
                //如果是某个街镇用户,直接赋予辖区初始值
                //if (FrmLogin.string辖区 != "顺德区")
                //{
                //    dgvComBox.Value = FrmLogin.string辖区;
                //}
                //else {
        ...
        private void InitialMap()//初使化地图
        {
            try
            {
                ...
                IMapLayer mapLayer = mapControl1.Map.Layers["道路面"];
                int ilayer = mapControl1.Map.Layers.IndexOf(mapLayer);
                mapControl1.Map.Layers.Move(ilayer, 3);

                //MIConnection miConnection = new MIConnection();
                //miConnection.Open();
                //TableInfoServer ti = new TableInfoServer("信息点", "SRVR=" + datasource + ";UID=" + userid + ";PWD=" + password, "Select  *  From 信息点", ServerToolkit.Oci);
                //ti.CacheSettings.CacheType = CacheOption.Off;
        ...
        private void CreateEagleLayer()
        {
            try
            {
                mapOverview.MouseWheelSupport = new MouseWheelSupport(MouseWheelBehavior.None, 10, 5); //鼠标滑轮默认是放大缩小的功能，这里将该功能取消
        ...
        private void mapControl1_ViewChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                iLevel = setToolslevel();   //显示级别
                //如果看影像，显示影像
                if (toolImgOrMap.Text == "地图")
                {
                    closeOtherLevelImg(iLevel);//先关闭其他级别的影像
        ...
        private void mapOverview_MouseDown(object sender, MouseEventArgs e)
        {
            try
            {
                MapInfo.Geometry.DPoint dP;//将鼠标点设为主地图和鹰眼图的中心
                    ...
                    case "toolImgOrMap":
                        try
                        {
                            if (e.ClickedItem.Text == "影像")
                            {
                                ...
                                mapControl1.Map.Layers["道路面"].Enabled = false;
                            ...
                            else
                            {
                                ...
                                mapControl1.Map.Layers["道路面"].Enabled = true;
                        ...
                        switch (this.toolGPSPolice.Text)
                        {
                            case "警员联动编辑":
                                ...
                                else
                                {
                                    ...
                                    this.toolGPSPolice.Text = "警员联动编辑";
        ...
        private void GetPolice(string policeNo)
        {
            try
            {
                ...
                this.toolGPSPolice.Text = "关闭联动编辑";
        ...
        private void UncheckedTool()//将ToolStripButton设置为未选中的状态
        {
            try
            {
        ...
        private void mapControl1_MouseMove(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                if (tabControl2.SelectedTab == tabPage1 && this.comboTables.Text=="民警中队辖区")
                {
                ...
                if (tabControl2.SelectedTab == tabPage1 && this.comboTables.Text == "警务室辖区")
                {
                    // 当鼠标移到某一个警务室时显示该警务室的名字
                    System.Drawing.Point point = e.Location;
                    MapInfo.Geometry.DPoint dpt;
        ...
        private void GetTable(string tableName)//从数据库中得到表
        {
            if (isOracleSpatialTab)
            {
                MIConnection miConnection = new MIConnection();
        ...
        private void Feature_Changed(object sender, MapInfo.Tools.FeatureChangedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    if (this.tabControl2.SelectedTab == tabPage1 && temEditDt.Rows[0]["└基础数据可删、改"].ToString() != "1")
                    {
                        this.Cursor = Cursors.Default;
                        MessageBox.Show("您没有删除权限！");
                        return;
                    }
                    if (this.tabControl2.SelectedTab == tabPage3 && temEditDt.Rows[0]["└业务数据可删、改"].ToString() != "1")
                    {
                        this.Cursor = Cursors.Default;
                        MessageBox.Show("您没有删除权限！");
                        return;
                    ...
                    {
                        if (tabControl2.SelectedTab == tabPage3 && comboTable.Text.Trim() == "人口系统")
                        {
                            try
                            {
                                int iID = 0;
                ...
                {
                    if (this.tabControl2.SelectedTab == tabPage1 && temEditDt.Rows[0]["基础数据可编辑"].ToString() != "1")
                    {
                        this.Cursor = Cursors.Default;
                        MessageBox.Show("您没有移动权限！");
                        return;
                    }
                    if (this.tabControl2.SelectedTab == tabPage3 && temEditDt.Rows[0]["业务数据可编辑"].ToString() != "1")
                    {
                        this.Cursor = Cursors.Default;
                        MessageBox.Show("您没有移动权限！");
                        return;
                    ...
                    if (tabControl2.SelectedTab == tabPage3)   //公安业务数据
                    {
                        MoveUpData(dataGridViewGaList, dataGridViewGaInfo, comboTable.Text, rowIndex);
                    }
                    else if (tabControl2.SelectedTab == tabPage1) //基础数据编辑
                    {
        ...
        //定义一个函数，用于在移动后直接更新移动要素 fisher in 09-12-31
        ...
        private void Feature_Selected(object sender, FeatureSelectedEventArgs e)
        {
            ...
            if (tabControl2.SelectedTab == tabPage3)   //公安业务编辑模块  by feng 09-08-21
            {
        ...
        private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)//添加语句，用来判断输入的格式正不正确
        {
            //OracleConnection Conn = new OracleConnection(mysqlstr);
            string type = "";//得到类型
            try
        ...
        private void GacheckNumber(string str)//判断输入的是不是数字,公安编辑模块  feng
        {
            try
            {
                str = str.Trim();
        ...
        private void checkFloat(string str)//判断输入的是不是数字
        {
            try
            {
                str = str.Trim();
        ...
        private void GacheckFloat(string str)//判断输入的是不是数字,公安编辑模块  feng
        {
            try
            {
                str = str.Trim();
        ...
        private void insertQueryIntoTable(DataTable dataTable)//将查询到的结果添加到表中
        {
            try
            {
            ...
            for (int i = 0; i < dataTable.Rows.Count; i++)
            {
                ...
                try
                {
                    ...
                    if (dX > 0 && dY > 0)
                    {
                        ...
                        //    if (tabName == "治安卡口系统")
                        //    {
                        //        pFeat["MapID"] = dataTable.Rows[i]["编号"].ToString();
                        //    }
                        //}
        ...
        private void frmMap_FormClosing(object sender, FormClosingEventArgs e) //关闭时，如果有对象未保存，提示
        {
            try
            {
        ...
        private void buttonSaveWZ_Click(object sender, EventArgs e)   //更新位置
        {
        ...
        private void buttonOK_Click(object sender, EventArgs e)       //update by fisher in 09-12-22
        {
            try
            {
                ...
                if (this.dataGridExp.Rows.Count == 0)
                {
                    if (strRegion == "顺德区")
                    {
                        //sql = "select 设备名称,X,Y,所属派出所,日常管理人,设备编号,mapid from 视频位置";
                        //videolocSql = "select 设备名称,X,Y,所属派出所,日常管理人,设备编号,mapid from 视频位置";
                    ...
                    {
                        //sql = "select 设备名称,X,Y,所属派出所,日常管理人,设备编号,mapid from 视频位置 where 所属派出所 in ('" + strRegion.Replace(",", "','") + "')";
                        //videolocSql = "select 设备名称,X,Y,所属派出所,日常管理人,设备编号,mapid from 视频位置 where 所属派出所 in ('" + strRegion.Replace(",", "','") + "')";
                ...
                else 
                {
                    ...
                    if (strRegion == "顺德区")
                    {
                        //sql = "select 设备名称,X,Y,所属派出所,日常管理人,设备编号,mapid from 视频位置 where " + Video_getSqlString();
                        //videolocSql = "select 设备名称,X,Y,所属派出所,日常管理人,设备编号,mapid from 视频位置 where " + Video_getSqlString();
                    ...
                    {
                        //sql = "select 设备名称,X,Y,所属派出所,日常管理人,设备编号,mapid from 视频位置 where " + Video_getSqlString() + " and 所属派出所 in ('" + strRegion.Replace(",", "','") + "')";
                        //videolocSql = "select 设备名称,X,Y,所属派出所,日常管理人,设备编号,mapid from 视频位置 where " + Video_getSqlString() + " and 所属派出所 in ('" + strRegion.Replace(",", "','") + "')";
                ...
                dataGridViewVideo.Columns[5].Visible = false;      // 设备编号不显示
                dataGridViewVideo.Columns[6].Visible = false;      // mapid不显示
                //dataGridViewVideo.Columns[4].ReadOnly = true;   //设备编号不允许修改
                //dataGridViewVideo.Columns[3].ReadOnly = true;   //mapid不允许修改
                insertQueryIntoTable(dt);
                this.toolEditPro.Value = 3;
                Application.DoEvents();
                isShowPro(false);
        ...
        private void tabControl2_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (tabControl2.SelectedTab == tabPage3) //公安业务编辑
            {
                 settabGa();//设置公安业务编辑的初值  feng
            ...
            try
            {
                ...
                if (dataGridView1.Rows.Count > 2)
                {
                    ...
                    switch (cellName)
                    {
                        ...
                        //    for (int i = 0; i < dataGridView1.Rows.Count; i++)
                        //    {
                        //        if (dataGridView1.Rows[i].Cells[0].Value.ToString() == "所属中队代码")
                        //        {
                        //            dataGridView1.Rows[i].Cells[1].Value = telNo;
                        //        }
                        //    }
                        ...
                        //    for (int i = 0; i < dataGridView1.Rows.Count; i++)
                        //    {
                        //        if (dataGridView1.Rows[i].Cells[0].Value.ToString() == "所属警务室代码")
                        //        {
                        //            dataGridView1.Rows[i].Cells[1].Value = telNo;
                        //        }
                        //    }
        ...
        int _end = 1;           // 开始行数
        string tabName = "";    // 表名
        string _mySQL = "", _PageSQL = "", _lctSQL = "";  //用户查询权限
        //-------------------------------------------------------------------

        string PageSQL = "";   //用于获得分页数据
        string lctSQL = "";    //用于已定位或未定位的SQL语句的传递，真繁琐（fisher）
        string mySQL = "";
        ...
        private void PageNow_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar.ToString() == "\r")
            {
                try
                {
                    ...
                    //{
                    //    MessageBox.Show("请输入数字！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information, MessageBoxDefaultButton.Button2);
                    //    this.PageNow.Text = pageCurrent.ToString();
                    //    return;
                    //}
                    else
        ...
        private void btnGaLocatedNo_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (Convert.ToInt32(dt.Rows[0][0]) < 1)
                {
                    ...
                    //catch (Exception ex)
                    //{
                    //    MessageBox.Show(ex.Message, "btnGaLocatedNo_Click关闭之前的表");
                    //}
        ...
        private void btnLocatedNo_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dt.Rows.Count < 1)
                {
                    ...
                    //catch (Exception ex)
                    //{
                    //    MessageBox.Show(ex.Message, "btnLocatedNo_Click()关闭之前的表");
                    //}
